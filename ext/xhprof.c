 ...
 */
zend_class_entry *dao_xhprof_ce;

PHP_METHOD(Dao_Xhprof, enable);
PHP_METHOD(Dao_Xhprof, disable);

ZEND_BEGIN_ARG_INFO_EX(arginfo_dao_xhprof_enable, 0, 0, 1)
    ZEND_ARG_TYPE_INFO(0, flags, IS_LONG, 0)
ZEND_END_ARG_INFO()

static const zend_function_entry dao_xhprof_method_entry[] = {
    PHP_ME(Dao_Xhprof, enable, arginfo_dao_xhprof_enable, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(Dao_Xhprof, disable, arginfo_empty, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_FE_END
};

/**
 * Dao\Xhprof initializer
 */
DAO_INIT_CLASS(Dao_Xhprof){

    DAO_REGISTER_CLASS(Dao, Xhprof, xhprof, dao_xhprof_method_entry, 0);

    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_MEMORY"),    DAO_XHPROF_FLAG_MEMORY);
    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_MEMORY_MU"), DAO_XHPROF_FLAG_MEMORY_MU);
    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_MEMORY_PMU"), DAO_XHPROF_FLAG_MEMORY_PMU);
    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_CPU"), DAO_XHPROF_FLAG_CPU);
    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_NO_BUILTINS"), DAO_XHPROF_FLAG_NO_BUILTINS);
    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_MEMORY_ALLOC"), DAO_XHPROF_FLAG_MEMORY_ALLOC);
    zend_declare_class_constant_long(dao_xhprof_ce, SL("FLAG_MEMORY_ALLOC_AS_MU"), DAO_XHPROF_FLAG_MEMORY_ALLOC_AS_MU);

    return SUCCESS;
}

ZEND_API void dao_xhprof_execute_internal(zend_execute_data *execute_data, zval *return_value) {
    int is_profiling = 1;

    if (!TXRG(enabled) || (TXRG(flags) & DAO_XHPROF_FLAG_NO_BUILTINS) > 0) {
...
ZEND_API void dao_xhprof_execute_ex (zend_execute_data *execute_data) {
...
void tracing_determine_clock_source() {
#ifdef __APPLE__
    TXRG(clock_source) = DAO_CLOCK_MACH;
#elif defined(__powerpc__) || defined(__ppc__)
    TXRG(clock_source) = DAO_CLOCK_TSC;
#else
    struct timespec res;
    ...
    if (TXRG(clock_use_rdtsc) == 1) {
        ...
        TXRG(clock_source) = DAO_CLOCK_CGT;
    } else {
        TXRG(clock_source) = DAO_CLOCK_GTOD;
    }
#endif
}
...
{
    TXRG(start_time) = dao_time_milliseconds(TXRG(clock_source), TXRG(timebase_factor));
    TXRG(start_timestamp) = dao_current_timestamp();
    TXRG(enabled) = 1;
    TXRG(root) = zend_string_init(DAO_XHPROF_ROOT_SYMBOL, sizeof(DAO_XHPROF_ROOT_SYMBOL)-1, 0);
...
void tracing_end()
{
    if (TXRG(enabled) == 1) {
        ...
        if (TXRG(flags) & DAO_XHPROF_FLAG_MEMORY_ALLOC) {
...
void tracing_callgraph_append_to_array(zval *return_value, zend_long flags)
{
    ...
    int as_mu = (TXRG(flags) & (DAO_XHPROF_FLAG_MEMORY_ALLOC_AS_MU | DAO_XHPROF_FLAG_MEMORY_MU)) == DAO_XHPROF_FLAG_MEMORY_ALLOC_AS_MU;
    ...
    for (i = 0; i < DAO_XHPROF_CALLGRAPH_SLOTS; i++) {
        ...
        while (bucket) {
			...
			if (flags) {
				...
				if (TXRG(flags) & DAO_XHPROF_FLAG_CPU) {
					add_assoc_long(stats, "cpu", bucket->cpu_time);
				}

				if (TXRG(flags) & DAO_XHPROF_FLAG_MEMORY_MU) {
					add_assoc_long(stats, "mu", bucket->memory);
				}

				if (TXRG(flags) & DAO_XHPROF_FLAG_MEMORY_PMU) {
...
void tracing_begin(zend_long flags)
{
    ...
    for (i = 0; i < DAO_XHPROF_CALLGRAPH_COUNTER_SIZE; i++) {
        TXRG(function_hash_counters)[i] = 0;
    }

    if (flags & DAO_XHPROF_FLAG_MEMORY_ALLOC) {
        zend_mm_heap *heap = zend_mm_get_heap();
...
{
    TXRG(timebase_factor) = dao_get_timebase_factor(TXRG(clock_source));
    TXRG(enabled) = 0;
    TXRG(flags) = 0;
...
PHP_METHOD(Dao_Xhprof, enable){
    ...
    if (!DAO_GLOBAL(xhprof).enable_xhprof) {
        DAO_THROW_EXCEPTION_STR(dao_exception_ce, "Change dao.xhprof.enable_xhprof in php.ini.");
        return;
    }
