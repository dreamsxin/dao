...
/**
 * Dao\Db\Dialect\Postgresql
 *
 * Generates database specific SQL for the PostgreSQL RBDMS
 */
zend_class_entry *dao_db_dialect_postgresql_ce;

PHP_METHOD(Dao_Db_Dialect_Postgresql, getColumnDefinition);
PHP_METHOD(Dao_Db_Dialect_Postgresql, addColumn);
PHP_METHOD(Dao_Db_Dialect_Postgresql, modifyColumn);
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropColumn);
PHP_METHOD(Dao_Db_Dialect_Postgresql, addIndex);
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropIndex);
PHP_METHOD(Dao_Db_Dialect_Postgresql, addPrimaryKey);
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropPrimaryKey);
PHP_METHOD(Dao_Db_Dialect_Postgresql, addForeignKey);
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropForeignKey);
PHP_METHOD(Dao_Db_Dialect_Postgresql, _getTableOptions);
PHP_METHOD(Dao_Db_Dialect_Postgresql, createTable);
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropTable);
PHP_METHOD(Dao_Db_Dialect_Postgresql, createView);
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropView);
PHP_METHOD(Dao_Db_Dialect_Postgresql, tableExists);
PHP_METHOD(Dao_Db_Dialect_Postgresql, viewExists);
PHP_METHOD(Dao_Db_Dialect_Postgresql, describeColumns);
PHP_METHOD(Dao_Db_Dialect_Postgresql, listTables);
PHP_METHOD(Dao_Db_Dialect_Postgresql, listViews);
PHP_METHOD(Dao_Db_Dialect_Postgresql, describeIndexes);
PHP_METHOD(Dao_Db_Dialect_Postgresql, describeReferences);
PHP_METHOD(Dao_Db_Dialect_Postgresql, tableOptions);
PHP_METHOD(Dao_Db_Dialect_Postgresql, getDefaultValue);

static const zend_function_entry dao_db_dialect_postgresql_method_entry[] = {
	PHP_ME(Dao_Db_Dialect_Postgresql, getColumnDefinition, arginfo_dao_db_dialectinterface_getcolumndefinition, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, addColumn, arginfo_dao_db_dialectinterface_addcolumn, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, modifyColumn, arginfo_dao_db_dialectinterface_modifycolumn, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, dropColumn, arginfo_dao_db_dialectinterface_dropcolumn, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, addIndex, arginfo_dao_db_dialectinterface_addindex, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, dropIndex, arginfo_dao_db_dialectinterface_dropindex, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, addPrimaryKey, arginfo_dao_db_dialectinterface_addprimarykey, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, dropPrimaryKey, arginfo_dao_db_dialectinterface_dropprimarykey, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, addForeignKey, arginfo_dao_db_dialectinterface_addforeignkey, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, dropForeignKey, arginfo_dao_db_dialectinterface_dropforeignkey, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, _getTableOptions, arginfo_empty, ZEND_ACC_PROTECTED)
	PHP_ME(Dao_Db_Dialect_Postgresql, createTable, arginfo_dao_db_dialectinterface_createtable, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, dropTable, arginfo_dao_db_dialectinterface_droptable, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, createView, arginfo_dao_db_dialectinterface_createview, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, dropView, arginfo_dao_db_dialectinterface_dropview, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, tableExists, arginfo_dao_db_dialectinterface_tableexists, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, viewExists, arginfo_dao_db_dialectinterface_viewexists, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, describeColumns, arginfo_dao_db_dialectinterface_describecolumns, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, listTables, arginfo_dao_db_dialectinterface_listtables, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, listViews, arginfo_dao_db_dialectinterface_listtables, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, describeIndexes, arginfo_dao_db_dialectinterface_describeindexes, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, describeReferences, arginfo_dao_db_dialectinterface_describereferences, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, tableOptions, arginfo_dao_db_dialectinterface_tableoptions, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Db_Dialect_Postgresql, getDefaultValue, arginfo_dao_db_dialectinterface_getdefaultvalue, ZEND_ACC_PUBLIC)
	PHP_FE_END
};

/**
 * Dao\Db\Dialect\Postgresql initializer
 */
DAO_INIT_CLASS(Dao_Db_Dialect_Postgresql){

	DAO_REGISTER_CLASS_EX(Dao\\Db\\Dialect, Postgresql, db_dialect_postgresql, dao_db_dialect_ce, dao_db_dialect_postgresql_method_entry, 0);

	zend_declare_property_string(dao_db_dialect_postgresql_ce, SL("_escapeChar"), "\"", ZEND_ACC_PROTECTED);

	zend_class_implements(dao_db_dialect_postgresql_ce, 1, dao_db_dialectinterface_ce);
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, getColumnDefinition){
	...
	if (Z_TYPE_P(column) != IS_OBJECT) {
		DAO_THROW_EXCEPTION_STR(dao_db_exception_ce, "Column definition must be an object compatible with Dao\\Db\\ColumnInterface");
		return;
	}

	DAO_CALL_METHOD(&size, column, "getsize");
	DAO_CALL_METHOD(&column_type, column, "gettype");
	DAO_CALL_METHOD(&isautoincrement, column, "isautoincrement");
	...
	if (Z_TYPE(column_type) == IS_STRING) {
		...
		switch (dao_get_intval(&column_type)) {
			...
			case DAO_DB_COLUMN_TYPE_JSONB:
				break;

			case DAO_DB_COLUMN_TYPE_BOOLEAN:
				DAO_SCONCAT_STR(&column_sql, "(1)");
				break;

			default:
				DAO_CALL_METHOD(&name, column, "getname");
				DAO_THROW_EXCEPTION_FORMAT(dao_db_exception_ce, "Unrecognized PostgreSQL data type at column %s", Z_STRVAL(name));
		}
		RETURN_ZVAL(&column_sql, 0, 0);
	}
	...
	switch (dao_get_intval(&column_type)) {
		...
		case DAO_DB_COLUMN_TYPE_VARCHAR:
			DAO_CONCAT_SVS(&column_sql, "CHARACTER VARYING(", &size, ")");
			break;
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, addColumn){
	...
	DAO_CONCAT_SVS(&sql, "ALTER TABLE ", &table, " ADD COLUMN ");
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, modifyColumn){
	...
	dao_fetch_params(1, 3, 1, &table_name, &schema_name, &column, &current_column);
	DAO_MM_VERIFY_INTERFACE_EX(column, dao_db_columninterface_ce, dao_db_exception_ce);

	DAO_MM_CALL_METHOD(&table, getThis(), "preparetable", table_name, schema_name);
	DAO_MM_ADD_ENTRY(&table);
	DAO_CONCAT_SV(&alter_table, "ALTER TABLE ", &table);
	DAO_MM_ADD_ENTRY(&alter_table);

	DAO_MM_CALL_METHOD(&name, column, "getname");
	DAO_MM_ADD_ENTRY(&name);
	DAO_MM_CALL_METHOD(&column_definition, getThis(), "getcolumndefinition", column);
	DAO_MM_ADD_ENTRY(&column_definition);
	DAO_MM_CALL_METHOD(&column_type, column, "gettype");
	DAO_MM_ADD_ENTRY(&column_type);

	if (!current_column) {
		DAO_SCONCAT_VSVSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" TYPE ", &column_definition, ";");
		DAO_MM_ADD_ENTRY(&sql);

		DAO_MM_CALL_METHOD(&is_not_null, column, "isnotnull");
		if (zend_is_true(&is_not_null)) {
			DAO_SCONCAT_VSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" SET NOT NULL;");
		} else {
			DAO_SCONCAT_VSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" DROP NOT NULL;");
		}
		DAO_MM_ADD_ENTRY(&sql);

		DAO_MM_CALL_METHOD(&default_value, column, "getdefaultvalue");
		if (Z_TYPE(default_value) == IS_NULL) {
			DAO_SCONCAT_VSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" DROP DEFAULT;");
		} else {
			DAO_MM_CALL_METHOD(&default_value, getThis(), "getdefaultvalue", &default_value, &column_type);
			DAO_MM_ADD_ENTRY(&default_value);
			DAO_SCONCAT_VSVSV(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" SET DEFAULT ", &default_value);
		}
		DAO_MM_ADD_ENTRY(&sql);
		RETURN_MM_CTOR(&sql);
	}

	DAO_MM_VERIFY_INTERFACE_EX(current_column, dao_db_columninterface_ce, dao_db_exception_ce);
	DAO_MM_CALL_METHOD(&current_name, current_column, "getname");
	DAO_MM_ADD_ENTRY(&current_name);

	if (!DAO_IS_EQUAL(&name, &current_name)) {
		DAO_CONCAT_VSVSVS(&sql, &alter_table, " RENAME COLUMN \"", &current_name, "\" TO \"", &name, "\";");
		DAO_MM_ADD_ENTRY(&sql);
	}

	DAO_MM_CALL_METHOD(&current_type, current_column, "gettype");
	DAO_MM_ADD_ENTRY(&current_type);

	if (!DAO_IS_IDENTICAL(&column_type, &current_type)) {
		DAO_SCONCAT_VSVSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" TYPE ", &column_definition, ";");
		DAO_MM_ADD_ENTRY(&sql);
	}

	DAO_MM_CALL_METHOD(&is_not_null, column, "isnotnull");
	DAO_MM_CALL_METHOD(&current_is_not_null, current_column, "isnotnull");
	if (!DAO_IS_EQUAL(&is_not_null, &current_is_not_null)) {
		if (zend_is_true(&is_not_null)) {
			DAO_SCONCAT_VSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" SET NOT NULL;");
		} else {
			DAO_SCONCAT_VSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" DROP NOT NULL;");
	...
	DAO_MM_CALL_METHOD(&default_value, column, "getdefaultvalue");
	DAO_MM_CALL_METHOD(&current_default_value, current_column, "getdefaultvalue");
	if (!DAO_IS_EQUAL(&default_value, &current_default_value)) {
		if (Z_TYPE(default_value) == IS_NULL) {
			DAO_SCONCAT_VSVS(&sql, &alter_table, " ALTER COLUMN \"", &name, "\" DROP DEFAULT;");
		} else {
			DAO_MM_CALL_METHOD(&column_type, column, "gettype");
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropColumn){
	...
	DAO_CALL_METHOD(&table, getThis(), "preparetable", table_name, schema_name);
	DAO_CONCAT_SVSVS(return_value, "ALTER TABLE ", &table, " DROP COLUMN \"", column_name, "\"");
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, addIndex){
	...
	if (dao_comparestr_str(&name, SL("PRIMARY"), &DAO_GLOBAL(z_false))) {
		DAO_RETURN_CALL_METHOD(getThis(), "addprimarykey", table_name, schema_name, index);
		return;
	}

	DAO_CALL_METHOD(&table, getThis(), "preparetable", table_name, schema_name);
	DAO_CALL_METHOD(&type, index, "gettype");
	DAO_CALL_METHOD(&columns, index, "getcolumns");
	DAO_CALL_METHOD(&columnlist, getThis(), "getcolumnlist", &columns);

	if (DAO_IS_NOT_EMPTY(&type)) {
		DAO_CONCAT_SVSVSVSVS(return_value, "CREATE ", &type, " INDEX \"", &name, "\" ON ", &table, " (", &columnlist, ")");
	} else {
		DAO_CONCAT_SVSVSVS(return_value, "CREATE INDEX \"", &name, "\" ON ", &table, " (", &columnlist, ")");
	}
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropIndex){
	...
	DAO_CONCAT_SVS(return_value, "DROP INDEX \"", index_name, "\"");
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, addPrimaryKey){
	...
	DAO_CALL_METHOD(&columns, index, "getcolumns");
	DAO_CALL_METHOD(&columnlist, getThis(), "getcolumnlist", &columns);
	DAO_CONCAT_SVSVS(return_value, "ALTER TABLE ", &table, " ADD CONSTRAINT \"PRIMARY\" PRIMARY KEY (", &columnlist, ")");
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropPrimaryKey){
	...
	DAO_CONCAT_SVS(return_value, "ALTER TABLE ", &table, " DROP CONSTRAINT \"PRIMARY\"");
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, addForeignKey){
	...
	DAO_CALL_METHOD(&columnlist, getThis(), "getcolumnlist", &columns);
	DAO_CALL_METHOD(&referenced_table, reference, "getreferencedtable", &columns);
	DAO_CALL_METHOD(&referenced_columns, reference, "getreferencedcolumns");
	DAO_CALL_METHOD(&referenced_columnlist, getThis(), "getcolumnlist", &referenced_columns);
	DAO_CALL_METHOD(&on_delete, reference, "getondelete");
	DAO_CALL_METHOD(&on_udpate, reference, "getonupdate");

	DAO_CONCAT_SVSVSVS(return_value, "ALTER TABLE ", &table, " ADD CONSTRAINT \"", &name, "\" FOREIGN KEY (", &columnlist, ")");
	DAO_SCONCAT_SVSVS(return_value, " REFERENCES \"", &referenced_table, "\" (", &referenced_columnlist, ")");

	if (DAO_IS_NOT_EMPTY(&on_delete)) {
		DAO_SCONCAT_SV(return_value, " ON DELETE ", &on_delete);
	}

	if (DAO_IS_NOT_EMPTY(&on_udpate)) {
		DAO_SCONCAT_SV(return_value, " ON UPDATE ", &on_udpate);
	}
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropForeignKey){
	...
	DAO_CONCAT_SVSVS(return_value, "ALTER TABLE ", &table, " DROP CONSTRAINT \"", reference_name, "\"");
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, createTable){
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&columns), column) {
		...
		if (zend_is_true(&attribute)) {
			dao_array_append(&primary_columns, &column_name, PH_COPY);
		}

		dao_array_append(&create_lines, &column_line, PH_COPY);
	} ZEND_HASH_FOREACH_END();

	if (dao_fast_count_ev(&primary_columns)) {
		DAO_CALL_METHOD(&primary_column_list, getThis(), "getcolumnlist", &primary_columns);
		DAO_CONCAT_SVS(&primary_sql, "PRIMARY KEY (", &primary_column_list, ")");
		dao_array_append(&create_lines, &primary_sql, PH_COPY);
	}
	...
	if (dao_array_isset_fetch_str(&indexes, definition, SL("indexes"), PH_READONLY)) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&indexes), index) {
			...
			if (DAO_IS_STRING(&index_name, "PRIMARY")) {
				DAO_CONCAT_SVS(&index_sql, "CONSTRAINT \"PRIMARY\" PRIMARY KEY (", &column_list, ")");
			} else if (DAO_IS_NOT_EMPTY(&index_type)) {
				DAO_CONCAT_SVSVSVS(&index_sql, "CONSTRAINT \"", &index_name, "\" ", &index_type, " (", &column_list, ")");
			} else {
				DAO_SCONCAT_SVSVSVS(&indexsql_aftercreate, "CREATE INDEX \"", &index_name, "\" ON ", &table, " (", &column_list, ");");
			}
	...
	DAO_SCONCAT_SV(&sql, ";", &indexsql_aftercreate);
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, dropTable){
	...
	if (zend_is_true(if_exists)) {
		DAO_CONCAT_SV(&sql, "DROP TABLE IF EXISTS ", &table);
	} else {
		DAO_CONCAT_SV(&sql, "DROP TABLE ", &table);
	}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, tableExists){
	...
	if (zend_is_true(schema_name)) {
		DAO_CONCAT_SVSVS(&sql, "SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM information_schema.tables WHERE table_schema = '", schema_name, "' AND table_name='", table_name, "'");
	} else {
		DAO_CONCAT_SVS(&sql, "SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM information_schema.tables WHERE table_schema = 'public' AND table_name='", table_name, "'");
	}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, viewExists){
	...
	if (zend_is_true(schema_name)) {
		DAO_CONCAT_SVSVS(return_value, "SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM pg_views WHERE viewname='", view_name, "' AND schemaname='", schema_name, "'");
	} else {
		DAO_CONCAT_SVS(return_value, "SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM pg_views WHERE viewname='", view_name, "'");
	}
}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, describeColumns){
	...
	if (zend_is_true(schema)) {
		DAO_CONCAT_SVSVS(&sql, "SELECT DISTINCT c.column_name AS Field, c.data_type AS Type, c.character_maximum_length AS Size, c.numeric_precision AS NumericSize, c.numeric_scale AS NumericScale, c.is_nullable AS Null, CASE WHEN pkc.column_name NOTNULL THEN 'PRI' ELSE '' END AS Key, CASE WHEN c.data_type LIKE '%int%' AND c.column_default LIKE '%nextval%' THEN 'auto_increment' ELSE c.column_default END AS Extra, c.ordinal_position AS Position, e.data_type AS element_type FROM information_schema.columns c LEFT JOIN ( SELECT kcu.column_name, kcu.table_name, kcu.table_schema FROM information_schema.table_constraints tc INNER JOIN information_schema.key_column_usage kcu on (kcu.constraint_name = tc.constraint_name and kcu.table_name=tc.table_name and kcu.table_schema=tc.table_schema) WHERE tc.constraint_type='PRIMARY KEY') pkc ON (c.column_name=pkc.column_name AND c.table_schema = pkc.table_schema AND c.table_name=pkc.table_name) LEFT JOIN information_schema.element_types e ON ((c.table_catalog, c.table_schema, c.table_name, 'TABLE', c.dtd_identifier) = (e.object_catalog, e.object_schema, e.object_name, e.object_type, e.collection_type_identifier)) WHERE c.table_schema='", schema, "' AND c.table_name='", table, "' ORDER BY c.ordinal_position");
	} else {
		DAO_CONCAT_SVS(&sql, "SELECT DISTINCT c.column_name AS Field, c.data_type AS Type, c.character_maximum_length AS Size, c.numeric_precision AS NumericSize, c.numeric_scale AS NumericScale, c.is_nullable AS Null, CASE WHEN pkc.column_name NOTNULL THEN 'PRI' ELSE '' END AS Key, CASE WHEN c.data_type LIKE '%int%' AND c.column_default LIKE '%nextval%' THEN 'auto_increment' ELSE c.column_default END AS Extra, c.ordinal_position AS Position, e.data_type AS element_type FROM information_schema.columns c LEFT JOIN ( SELECT kcu.column_name, kcu.table_name, kcu.table_schema FROM information_schema.table_constraints tc INNER JOIN information_schema.key_column_usage kcu on (kcu.constraint_name = tc.constraint_name and kcu.table_name=tc.table_name and kcu.table_schema=tc.table_schema) WHERE tc.constraint_type='PRIMARY KEY') pkc ON (c.column_name=pkc.column_name AND c.table_schema = pkc.table_schema AND c.table_name=pkc.table_name) LEFT JOIN information_schema.element_types e ON ((c.table_catalog, c.table_schema, c.table_name, 'TABLE', c.dtd_identifier) = (e.object_catalog, e.object_schema, e.object_name, e.object_type, e.collection_type_identifier)) WHERE c.table_schema='public' AND c.table_name='", table, "' ORDER BY c.ordinal_position");
	}
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, listTables){
	...
	if (zend_is_true(schema_name)) {
		DAO_CONCAT_SVS(&sql, "SELECT table_name FROM information_schema.tables WHERE table_schema = '", schema_name, "' ORDER BY table_name");
	} else {
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, listViews){
	...
	if (zend_is_true(schema_name)) {
		DAO_CONCAT_SVS(&sql, "SELECT viewname AS view_name FROM pg_views WHERE schemaname = '", schema_name, "' ORDER BY view_name");
	} else {
...
PHP_METHOD(Dao_Db_Dialect_Postgresql, describeReferences){
	...
	if (zend_is_true(schema)) {
		DAO_SCONCAT_SVSVS(&sql, "tc.table_schema = '", schema, "' AND tc.table_name='", table, "'");
	} else {
		DAO_SCONCAT_SVS(&sql, "tc.table_name='", table, "'");
	}
