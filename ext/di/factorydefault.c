 ...
 */
zend_class_entry *dao_di_factorydefault_ce;

PHP_METHOD(Dao_Di_FactoryDefault, __construct);

static const zend_function_entry dao_di_factorydefault_method_entry[] = {
	PHP_ME(Dao_Di_FactoryDefault, __construct, arginfo_empty, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
	PHP_FE_END
};


/**
 * Dao\Di\FactoryDefault initializer
 */
DAO_INIT_CLASS(Dao_Di_FactoryDefault){

	DAO_REGISTER_CLASS_EX(Dao\\Di, FactoryDefault, di_factorydefault, dao_di_ce, dao_di_factorydefault_method_entry, 0);
...
PHP_METHOD(Dao_Di_FactoryDefault, __construct){
	...
	if (name) {
		DAO_CALL_PARENT(NULL, dao_di_factorydefault_ce, getThis(), "__construct", name);
	} else {
		DAO_CALL_PARENT(NULL, dao_di_factorydefault_ce, getThis(), "__construct");
	}
	...
	ZVAL_STRING(&definition, "Dao\\Mvc\\Router");
	...
	ZVAL_STRING(&definition, "Dao\\Mvc\\Dispatcher");
	DAO_CALL_SELF(NULL, "set", &servicename, &definition, &DAO_GLOBAL(z_true));
	zval_ptr_dtor(&definition);
	...
	ZVAL_STRING(&definition, "Dao\\Assets\\Manager");
	DAO_CALL_SELF(NULL, "set", &servicename, &definition, &DAO_GLOBAL(z_true));
	zval_ptr_dtor(&definition);
}
