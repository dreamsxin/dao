 ...
 *</code>
 */
zend_class_entry *dao_validation_validator_digit_ce;

PHP_METHOD(Dao_Validation_Validator_Digit, validate);
PHP_METHOD(Dao_Validation_Validator_Digit, valid);

static const zend_function_entry dao_validation_validator_digit_method_entry[] = {
	PHP_ME(Dao_Validation_Validator_Digit, validate, arginfo_dao_validation_validatorinterface_validate, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Validation_Validator_Digit, valid, arginfo_dao_validation_validatorinterface_valid, ZEND_ACC_PUBLIC)
	PHP_FE_END
};

/**
 * Dao\Validation\Validator\Digit initializer
 */
DAO_INIT_CLASS(Dao_Validation_Validator_Digit){

	DAO_REGISTER_CLASS_EX(Dao\\Validation\\Validator, Digit, validation_validator_digit, dao_validation_validator_ce, dao_validation_validator_digit_method_entry, 0);

	zend_class_implements(dao_validation_validator_digit_ce, 1, dao_validation_validatorinterface_ce);
...
PHP_METHOD(Dao_Validation_Validator_Digit, validate){
	...
	if (DAO_IS_FALSE(&valid)) {
		...
		if (!zend_is_true(&message_str)) {
			RETURN_ON_FAILURE(dao_validation_getdefaultmessage_helper(&message_str, Z_OBJCE_P(validaton), validaton, "Digit"));
		}
		...
		dao_validation_message_construct_helper(&message, &prepared, attribute, "Digit", &code);
...
PHP_METHOD(Dao_Validation_Validator_Digit, valid){
	...
	if (Z_TYPE_P(value) != IS_LONG) {
		DAO_CALL_FUNCTION(&valid, "ctype_digit", value);
