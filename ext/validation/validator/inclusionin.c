 ...
 *</code>
 */
zend_class_entry *dao_validation_validator_inclusionin_ce;

PHP_METHOD(Dao_Validation_Validator_InclusionIn, validate);
PHP_METHOD(Dao_Validation_Validator_InclusionIn, valid);

static const zend_function_entry dao_validation_validator_inclusionin_method_entry[] = {
	PHP_ME(Dao_Validation_Validator_InclusionIn, validate, arginfo_dao_validation_validatorinterface_validate, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Validation_Validator_InclusionIn, valid, arginfo_dao_validation_validatorinterface_valid, ZEND_ACC_PUBLIC)
	PHP_FE_END
};

/**
 * Dao\Validation\Validator\InclusionIn initializer
 */
DAO_INIT_CLASS(Dao_Validation_Validator_InclusionIn){

	DAO_REGISTER_CLASS_EX(Dao\\Validation\\Validator, InclusionIn, validation_validator_inclusionin, dao_validation_validator_ce, dao_validation_validator_inclusionin_method_entry, 0);

	zend_class_implements(dao_validation_validator_inclusionin_ce, 1, dao_validation_validatorinterface_ce);
...
PHP_METHOD(Dao_Validation_Validator_InclusionIn, validate)
{
	...
	if (DAO_IS_FALSE(&valid)) {
		...
		if (!zend_is_true(&message_str)) {
			RETURN_ON_FAILURE(dao_validation_getdefaultmessage_helper(&message_str, Z_OBJCE_P(validaton), validaton, "InclusionIn"));
		}
		...
		dao_validation_message_construct_helper(&message, &prepared, attribute, "InclusionIn", &code);
