 ...
 *</code>
 */
zend_class_entry *dao_validation_validator_numericality_ce;

PHP_METHOD(Dao_Validation_Validator_Numericality, validate);
PHP_METHOD(Dao_Validation_Validator_Numericality, valid);

static const zend_function_entry dao_validation_validator_numericality_method_entry[] = {
	PHP_ME(Dao_Validation_Validator_Numericality, validate, arginfo_dao_validation_validatorinterface_validate, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Validation_Validator_Numericality, valid, arginfo_dao_validation_validatorinterface_valid, ZEND_ACC_PUBLIC)
	PHP_FE_END
};

/**
 * Dao\Validation\Validator\Numericality initializer
 */
DAO_INIT_CLASS(Dao_Validation_Validator_Numericality){

	DAO_REGISTER_CLASS_EX(Dao\\Validation\\Validator, Numericality, validation_validator_numericality, dao_validation_validator_ce, dao_validation_validator_numericality_method_entry, 0);

	zend_class_implements(dao_validation_validator_numericality_ce, 1, dao_validation_validatorinterface_ce);
...
PHP_METHOD(Dao_Validation_Validator_Numericality, validate){
	...
	if (DAO_IS_FALSE(&valid)) {
		...
		if (!zend_is_true(&message_str)) {
			RETURN_ON_FAILURE(dao_validation_getdefaultmessage_helper(&message_str, Z_OBJCE_P(validaton), validaton, "Numericality"));
		}
		...
		dao_validation_message_construct_helper(&message, &prepared, attribute, "Numericality", &code);
