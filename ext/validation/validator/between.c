 ...
 *</code>
 */
zend_class_entry *dao_validation_validator_between_ce;

PHP_METHOD(Dao_Validation_Validator_Between, validate);
PHP_METHOD(Dao_Validation_Validator_Between, valid);

static const zend_function_entry dao_validation_validator_between_method_entry[] = {
	PHP_ME(Dao_Validation_Validator_Between, validate, arginfo_dao_validation_validatorinterface_validate, ZEND_ACC_PUBLIC)
	PHP_ME(Dao_Validation_Validator_Between, valid, arginfo_dao_validation_validatorinterface_valid, ZEND_ACC_PUBLIC)
	PHP_FE_END
};

/**
 * Dao\Validation\Validator\Between initializer
 */
DAO_INIT_CLASS(Dao_Validation_Validator_Between){

	DAO_REGISTER_CLASS_EX(Dao\\Validation\\Validator, Between, validation_validator_between, dao_validation_validator_ce, dao_validation_validator_between_method_entry, 0);

	zend_class_implements(dao_validation_validator_between_ce, 1, dao_validation_validatorinterface_ce);
...
PHP_METHOD(Dao_Validation_Validator_Between, validate){
	...
	RETURN_ON_FAILURE(dao_validation_validator_getoption_helper(&minimum, ce, getThis(), "minimum"));
	RETURN_ON_FAILURE(dao_validation_validator_getoption_helper(&maximum, ce, getThis(), "maximum"));
	...
	if (DAO_IS_FALSE(&valid)) {
		...
		if (!zend_is_true(&message_str)) {
			RETURN_ON_FAILURE(dao_validation_getdefaultmessage_helper(&message_str, Z_OBJCE_P(validaton), validaton, "Between"));
		}
		...
		dao_validation_message_construct_helper(&message, &prepared, attribute, "Between", &code);
...
PHP_METHOD(Dao_Validation_Validator_Between, valid){
	...
	dao_fetch_params(0, 3, 0, &value, &minimum, &maximum);
