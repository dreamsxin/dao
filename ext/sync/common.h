...
#ifndef DAO_SYNC_COMMON_H
#define DAO_SYNC_COMMON_H
...
/* POSIX pthreads. */
static inline pthread_t dao_getcurrent_threadid()
{
	return pthread_self();
}

static inline uint64_t dao_getmicrosecondtime()
{
...
static inline int dao_clock_gettime(struct timespec *ts)
{
#ifdef __APPLE__
	clock_serv_t cclock;
	mach_timespec_t mts;
...
static inline size_t dao_getsystemalignmentsize()
{
	struct {
		int MxInt;
	} x;
...
static inline size_t dao_getalignsize(size_t Size)
{
	size_t AlignSize = dao_getsystemalignmentsize();
...
static inline int dao_namedmem_init(char **ResultMem, size_t *StartPos, const char *Prefix, const char *Name, size_t Size)
{
	int Result = -1;
	*ResultMem = NULL;
	*StartPos = (Name != NULL ? dao_getalignsize(1) + dao_getalignsize(sizeof(pthread_mutex_t)) + dao_getalignsize(sizeof(uint32_t)) : 0);
	...
	if (Name == NULL) {
		*ResultMem = (char *)ecalloc(1, Size);
		...
		sprintf(Nums, "-%u-%u-", (unsigned int)dao_getsystemalignmentsize(), (unsigned int)Size);
			...
			*ResultMem = (char *)mmap(NULL, Size, PROT_READ | PROT_WRITE, MAP_SHARED, fp, 0);
			if ((*ResultMem) == MAP_FAILED) {
				*ResultMem = NULL;
				...
				MutexPtr = (pthread_mutex_t *)((*ResultMem) + dao_getalignsize(1));
				RefCountPtr = (uint32_t *)((*ResultMem) + dao_getalignsize(1) + dao_getalignsize(sizeof(pthread_mutex_t)));
			...
			if (fp > -1) {
				...
				*ResultMem = (char *)mmap(NULL, Size, PROT_READ | PROT_WRITE, MAP_SHARED, fp, 0);
				if (*ResultMem == MAP_FAILED) {
					...
					char *MemPtr = (*ResultMem) + dao_getalignsize(1);
					MutexPtr = (pthread_mutex_t *)(MemPtr);
					MemPtr += dao_getalignsize(sizeof(pthread_mutex_t));
...
static inline void dao_namedmem_ready(char *MemPtr)
{
	pthread_mutex_unlock((pthread_mutex_t *)(MemPtr + dao_getalignsize(1)));
}

static inline void dao_namedmem_unmap(char *MemPtr, size_t Size)
{
	pthread_mutex_t *MutexPtr;
	uint32_t *RefCountPtr;

	char *MemPtr2 = MemPtr + dao_getalignsize(1);
	MutexPtr = (pthread_mutex_t *)(MemPtr2);
	MemPtr2 += dao_getalignsize(sizeof(pthread_mutex_t));
	...
	munmap(MemPtr, dao_getalignsize(1) + dao_getalignsize(sizeof(pthread_mutex_t)) + dao_getalignsize(sizeof(uint32_t)) + Size);
}

/* Basic *NIX Semaphore functions. */
static inline size_t dao_semaphore_getsize()
{
	return dao_getalignsize(sizeof(pthread_mutex_t)) + dao_getalignsize(sizeof(uint32_t)) + dao_getalignsize(sizeof(uint32_t)) + dao_getalignsize(sizeof(pthread_cond_t));
}

static inline void dao_semaphore_get(dao_semaphore_wrapper *Result, char *Mem)
{
	Result->MxMutex = (pthread_mutex_t *)(Mem);
	Mem += dao_getalignsize(sizeof(pthread_mutex_t));

	Result->MxCount = (uint32_t *)(Mem);
	Mem += dao_getalignsize(sizeof(uint32_t));
...
static inline void dao_semaphore_init(dao_semaphore_wrapper *UnixSemaphore, int Shared, uint32_t Start, uint32_t Max)
{
	pthread_mutexattr_t MutexAttr;
...
static inline int dao_semaphore_wait(dao_semaphore_wrapper *UnixSemaphore, uint32_t Wait)
{
	if (Wait == 0) {
	...
	if (UnixSemaphore->MxCount[0]) {
		...
		if (dao_clock_gettime(&TempTime) == -1)  return 0;
...
static inline int dao_semaphore_release(dao_semaphore_wrapper *UnixSemaphore, uint32_t *PrevVal)
{
	if (pthread_mutex_lock(UnixSemaphore->MxMutex) != 0)  return 0;
...
static inline void dao_semaphore_free(dao_semaphore_wrapper *UnixSemaphore)
{
...
static inline size_t dao_event_getsize()
{
	return dao_getalignsize(sizeof(pthread_mutex_t)) + dao_getalignsize(2) + dao_getalignsize(sizeof(uint32_t)) + dao_getalignsize(sizeof(pthread_cond_t));
}

static inline void dao_event_get(dao_event_wrapper *Result, char *Mem)
{
	Result->MxMutex = (pthread_mutex_t *)(Mem);
	Mem += dao_getalignsize(sizeof(pthread_mutex_t));
...
static inline void dao_event_init(dao_event_wrapper *UnixEvent, int Shared, int Manual, int Signaled)
{
	pthread_mutexattr_t MutexAttr;
	pthread_condattr_t CondAttr;
...
static inline int dao_event_wait(dao_event_wrapper *UnixEvent, uint32_t Wait)
{
	if (Wait == 0) {
		/* Avoid the scenario of deadlock on the semaphore itself for 0 wait. */
	...
	if (UnixEvent->MxSignaled[0] != '\x00' && (UnixEvent->MxManual[0] != '\x00' || !UnixEvent->MxWaiting[0])) {
		...
		if (dao_clock_gettime(&TempTime) == -1) {
...
static inline int dao_event_fire(dao_event_wrapper *UnixEvent)
{
	if (pthread_mutex_lock(UnixEvent->MxMutex) != 0)  return 0;
...
static inline int dao_event_reset(dao_event_wrapper *UnixEvent)
{
...
static inline void dao_event_free(dao_event_wrapper *UnixEvent)
{
	pthread_mutex_destroy(UnixEvent->MxMutex);
	pthread_cond_destroy(UnixEvent->MxCond);
}

#endif /* DAO_SYNC_COMMON_H */
