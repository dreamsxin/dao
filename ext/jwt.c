...
PHP_METHOD(Dao_JWT, encode);
PHP_METHOD(Dao_JWT, decode);

ZEND_BEGIN_ARG_INFO_EX(arginfo_dao_jwt_encode, 0, 0, 2)
    ZEND_ARG_ARRAY_INFO(0, payload, 1)
    ZEND_ARG_TYPE_INFO(0, key, IS_STRING, 1)
    ZEND_ARG_TYPE_INFO(0, alg, IS_STRING, 1)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_dao_jwt_decode, 0, 0, 2)
    ZEND_ARG_TYPE_INFO(0, token, IS_STRING, 1)
    ZEND_ARG_TYPE_INFO(0, key, IS_STRING, 1)
    ZEND_ARG_INFO(0, options)
ZEND_END_ARG_INFO()

static const zend_function_entry dao_jwt_method_entry[] = {
	PHP_ME(Dao_JWT, encode,   arginfo_dao_jwt_encode, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
	PHP_ME(Dao_JWT, decode, arginfo_dao_jwt_decode, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
	PHP_FE_END
};
 ...
 */
DAO_INIT_CLASS(Dao_JWT){

	DAO_REGISTER_CLASS(Dao, JWT, jwt, dao_jwt_method_entry, 0);

    /* register exception class */
    jwt_signature_invalid_cex = jwt_register_class("Dao\\JWT\\SignatureInvalidException");
    jwt_before_valid_cex = jwt_register_class("Dao\\JWT\\BeforeValidException");
    jwt_expired_signature_cex = jwt_register_class("Dao\\JWT\\ExpiredSignatureException");
    jwt_invalid_issuer_cex = jwt_register_class("Dao\\JWT\\InvalidIssuerException");
    jwt_invalid_aud_cex = jwt_register_class("Dao\\JWT\\InvalidAudException");
    jwt_invalid_jti_cex = jwt_register_class("Dao\\JWT\\InvalidJtiException");
    jwt_invalid_iat_cex = jwt_register_class("Dao\\JWT\\InvalidIatException");
    jwt_invalid_sub_cex = jwt_register_class("Dao\\JWT\\InvalidSubException");
