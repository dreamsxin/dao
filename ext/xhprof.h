...
#define DAO_XHPROF_ROOT_SYMBOL "main()"
#define DAO_XHPROF_FLAG_CPU 1
#define DAO_XHPROF_FLAG_MEMORY_MU 2
#define DAO_XHPROF_FLAG_MEMORY_PMU 4
#define DAO_XHPROF_FLAG_MEMORY 6
#define DAO_XHPROF_FLAG_MEMORY_ALLOC 16
#define DAO_XHPROF_FLAG_MEMORY_ALLOC_AS_MU (32|16)
#define DAO_XHPROF_FLAG_NO_BUILTINS 8
...
zend_always_inline static int tracing_enter_frame_callgraph(zend_string *root_symbol, zend_execute_data *execute_data)
{
    ...
    current_frame->wt_start = dao_time_milliseconds(TXRG(clock_source), TXRG(timebase_factor));

    if (TXRG(flags) & DAO_XHPROF_FLAG_CPU) {
        current_frame->cpu_start = dao_cpu_timer();
    }
    ...
    if ((TXRG(flags) & DAO_XHPROF_FLAG_MEMORY_MU) || (TXRG(flags) & DAO_XHPROF_FLAG_MEMORY_ALLOC_AS_MU)) {
    ...
    current_frame->hash_code = ZSTR_HASH(function_name) % DAO_XHPROF_CALLGRAPH_COUNTER_SIZE;
...
zend_always_inline static void tracing_exit_frame_callgraph()
{
    ...
    zend_long duration = dao_time_milliseconds(TXRG(clock_source), TXRG(timebase_factor)) - current_frame->wt_start;

    zend_ulong key = tracing_callgraph_bucket_key(current_frame);
    unsigned int slot = (unsigned int)key % DAO_XHPROF_CALLGRAPH_SLOTS;
    ...
    if (TXRG(flags) & DAO_XHPROF_FLAG_CPU) {
        bucket->cpu_time += (dao_cpu_timer() - current_frame->cpu_start);
    }
...
extern zend_class_entry *dao_xhprof_ce;
