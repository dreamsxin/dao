...
PHP_METHOD(Dao_Files, getExtension);
PHP_METHOD(Dao_Files, setExtension);
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_dao_files_getextension, 0, 0, 1)
	ZEND_ARG_TYPE_INFO(0, filename, IS_STRING, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_dao_files_setextension, 0, 0, 2)
	ZEND_ARG_TYPE_INFO(0, filename, IS_STRING, 0)
...
static const zend_function_entry dao_files_method_entry[] = {
	...
	PHP_ME(Dao_Files, list, arginfo_dao_files_list, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(Dao_Files, getExtension, arginfo_dao_files_getextension, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(Dao_Files, setExtension, arginfo_dao_files_setextension, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_FE_END
};
...
DAO_INIT_CLASS(Dao_Files){
	...
	DAO_REGISTER_CLASS(Dao, Files, files, dao_files_method_entry, 0);

	zend_declare_class_constant_long(dao_files_ce, SL("TYPE_NONE"), 0);
	zend_declare_class_constant_long(dao_files_ce, SL("TYPE_FILE"), 1);
	zend_declare_class_constant_long(dao_files_ce, SL("TYPE_DIR"),  2);
	return SUCCESS;
}
 ...
 * @return boolean
 */
PHP_METHOD(Dao_Files, RemoveDirectory){
	char *dir;
	size_t dir_len;
	zval *zcontext = NULL;
	php_stream_context *context;
...
PHP_METHOD(Dao_Files, delete){
	...
	dao_fetch_params(0, 1, 1, &path, &recursive);
	if (recursive == NULL) {
		recursive = &DAO_GLOBAL(z_false);
	}
	if (dao_is_file(path)) {
		dao_unlink(return_value, path);
		return;
	}
...
PHP_METHOD(Dao_Files, list){
    ...
    while (php_stream_readdir(stream, &entry)) {
		...
		dao_path_concat(&path, dirname, dirname_len, entry.d_name, strlen(entry.d_name));
		switch (mode) {
			case 1:	// file
				if (dao_is_file(&path)) {
