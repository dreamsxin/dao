...
int dao_static_property_array_isset_ce(zend_class_entry *ce, const char *property, uint32_t property_length, const zval *index)
{
	...
	dao_read_static_property_ce(&property_value, ce, property, property_length, PH_READONLY);

	if (Z_TYPE(property_value) != IS_ARRAY || !dao_array_isset(&property_value, index)) {
		return 0;
	}
	return 1;
}
...
int dao_read_static_property(zval *return_value, const char *class_name, uint32_t class_length, const char *property_name, uint32_t property_length, int flags)
{
	...
	if ((ce = zend_lookup_class(zend_string_init(class_name, class_length, 0))) != NULL) {
		return dao_read_static_property_ce(return_value, ce, property_name, property_length, flags);
	}
...
int dao_read_static_property_array_ce(zval *return_value, zend_class_entry *ce, const char *property, uint32_t property_length, const zval *index, int flags)
{
	zval arr = {};

	dao_read_static_property_ce(&arr, ce, property, property_length, PH_READONLY);

	if (Z_TYPE(arr) != IS_ARRAY || !dao_array_isset_fetch(return_value, &arr, index, flags)) {
		ZVAL_NULL(return_value);
		return 0;
	}
...
int dao_update_static_property_array_ce(zend_class_entry *ce, const char *property, uint32_t property_length, const zval *index, zval *value)
{
	...
	dao_read_static_property_ce(&tmp, ce, property, property_length, PH_READONLY);

	if (Z_TYPE(tmp) != IS_ARRAY) {
		array_init(&tmp);
		dao_array_update(&tmp, index,  value, PH_COPY);
		dao_update_static_property_ce(ce, property, property_length, &tmp);
		zval_ptr_dtor(&tmp);
		return SUCCESS;
	}
...
int dao_update_static_property_array_multi_ce(zend_class_entry *ce, const char *property, uint32_t property_length, zval *value, const char *types, int types_length, int types_count, ...)
{
	...
	dao_array_update_multi_ex(&tmp, value, types, types_length, types_count, ap);
	va_end(ap);

	if (separated) {
		flag = dao_update_static_property_ce(ce, property, property_length, &tmp);
	}
...
int dao_update_static_property_ce(zend_class_entry *ce, const char *property_name, uint32_t property_length, zval *value)
{
	zval garbage;
	zval *property;
...
int dao_update_static_property_bool_ce(zend_class_entry *ce, const char *property_name, uint32_t property_length, int value)
{
	zval v = {};
	ZVAL_BOOL(&v, value);
	return dao_update_static_property_ce(ce, property_name, property_length, &v);
}
...
int dao_update_static_property_array_append_ce(zend_class_entry *ce, const char *property, uint32_t property_length, zval *value)
{
	zval tmp = {};
	int separated = 0;
	...
	dao_array_append(&tmp, value, PH_COPY);
...
int dao_static_property_incr_ce(zend_class_entry *ce, const char *property, uint32_t len){
	...
	dao_read_static_property_ce(&value, ce, property, len, PH_READONLY);
	dao_increment(&value);
	dao_update_static_property_ce(ce, property, len, &value);
	return SUCCESS;
}
...
int dao_unset_static_property_array_ce(zend_class_entry *ce, const char *property, uint32_t property_length, const zval *index) {
	...
	dao_read_static_property_ce(&tmp, ce, property, property_length, PH_NOISY | PH_READONLY);

	/** Convert the value to array if not is an array */
	if (Z_TYPE(tmp) == IS_ARRAY) {
		dao_array_unset(&tmp, index, 0);
	}
...
void dao_get_class(zval *result, const zval *object, int lower) {
	...
	if (Z_TYPE_P(object) == IS_OBJECT) {
		...
		if (lower) {
			dao_strtolower_inplace(result);
		}

	} else if (Z_TYPE_P(object) == IS_STRING && dao_class_exists_ex(object, 1)) {
		ZVAL_COPY(result, object);
	} else {
		ZVAL_NULL(result);
		php_error_docref(NULL, E_WARNING, "dao_get_class expects an object");
	}
}
...
void dao_get_class_ns(zval *result, const zval *object, int lower) {
	...
	if (Z_TYPE_P(object) != IS_OBJECT) {
		if (Z_TYPE_P(object) != IS_STRING) {
			ZVAL_NULL(result);
			php_error_docref(NULL, E_WARNING, "dao_get_class_ns expects an object");
			return;
		}
	}
...
void dao_get_ns_class(zval *result, const zval *object, int lower) {
	...
	if (Z_TYPE_P(object) != IS_OBJECT) {
		if (Z_TYPE_P(object) != IS_STRING) {
			php_error_docref(NULL, E_WARNING, "dao_get_ns_class expects an object");
			ZVAL_NULL(result);
			return;
		}
	}
...
void dao_get_called_class(zval *return_value)
{
	...
	if (!zend_get_executed_scope())  {
		php_error_docref(NULL, E_WARNING, "dao_get_called_class() called from outside a class");
	}
#else
...
void dao_get_object_vars(zval *result, zval *object, int check_access) {
	...
	if (Z_TYPE_P(object) == IS_OBJECT) {
		...
		php_error_docref(NULL, E_WARNING, "dao_get_object_vars expects an object");
	}
}
...
void dao_get_object_members(zval *result, zval *object, int check_access) {
	...
	if (Z_TYPE_P(object) == IS_OBJECT) {
		...
		ZEND_HASH_FOREACH_KEY(properties, num_key, key) {
			...
			if (UNEXPECTED(!key)) {
				dao_array_append_long(result, num_key, 0);
			} else if (!is_dynamic && ZSTR_VAL(key)[0] == 0) {
				const char *prop_name, *class_name;
				size_t prop_len;
				zend_unmangle_property_name_ex(key, &class_name, &prop_name, &prop_len);
				dao_array_append_str(result, prop_name, prop_len, 0);
			} else {
				dao_array_append_str(result, ZSTR_VAL(key), ZSTR_LEN(key), 0);
			}
		} ZEND_HASH_FOREACH_END();
	} else {
 ...
 */
void dao_get_class_methods(zval *return_value, zval *object, int check_access) {
...
zend_class_entry *dao_fetch_class(const zval *class_name, int fetch_type) {
	...
	return dao_fetch_str_class(SL("stdclass"), fetch_type);
}
...
zend_class_entry *dao_class_exists_ex(const zval *class_name, int autoload) {
	...
	if (Z_TYPE_P(class_name) == IS_STRING) {
		return dao_class_exists(class_name, autoload);
	}
...
zend_class_entry *dao_class_str_exists(const char *class_name, uint32_t class_len, int autoload) {
	...
	ce = dao_class_exists(&name, autoload);
	zval_ptr_dtor(&name);
	return ce;
}
 ...
 */
int dao_interface_exists(const zval *class_name, int autoload) {
...
int dao_is_instance_of(zval *object, const char *class_name, unsigned int class_length) {
	...
	if (Z_TYPE_P(object) == IS_OBJECT) {
		...
		temp_ce = dao_fetch_str_class(class_name, class_length, ZEND_FETCH_CLASS_DEFAULT);
		if (temp_ce) {
 ...
 */
int dao_update_property(zval *object, const char *property_name, uint32_t property_length, zval *value){
 ...
 */
int dao_update_property_long(zval *object, const char *property_name, uint32_t property_length, long value)
{
	zval v = {};
	int ret;
	ZVAL_LONG(&v, value);
	ret = dao_update_property(object, property_name, property_length, &v);
	zval_ptr_dtor(&v);
	return ret;
}
...
int dao_update_property_str(zval *object, const char *property_name, uint32_t property_length, const char *str, uint32_t str_length)
{
	...
	status = dao_update_property(object, property_name, property_length, &tmp);
	zval_ptr_dtor(&tmp);
	return status;
}
 ...
 */
int dao_update_property_bool(zval *object, const char *property_name, uint32_t property_length, int value) {
	zval v = {};
	int ret;
	ZVAL_BOOL(&v, value);
	ret = dao_update_property(object, property_name, property_length, &v);
 ...
 */
int dao_update_property_null(zval *object, const char *property_name, uint32_t property_length) {
	zval v = {};
	int ret;
	ZVAL_NULL(&v);
	ret = dao_update_property(object, property_name, property_length, &v);
...
int dao_update_property_string_zval(zval *object, zend_string *property, zval *value){

	return dao_update_property(object, ZSTR_VAL(property), ZSTR_LEN(property), value);
}

int dao_update_property_zval_null(zval *object, const zval *property)
{
	zval v = {};
	int ret;
	ZVAL_NULL(&v);
	ret = dao_update_property(object, Z_STRVAL_P(property), Z_STRLEN_P(property), &v);
	zval_ptr_dtor(&v);
	return ret;
}
 ...
 */
int dao_update_property_zval_long(zval *object, const zval *property, int value)
{
	zval v = {};
	int ret;
	ZVAL_LONG(&v, value);
 ...
 */
int dao_update_property_zval_zval(zval *object, const zval *property, zval *value)
{
	...
	return dao_update_property(object, Z_STRVAL_P(property), Z_STRLEN_P(property), value);
}
...
int dao_update_property_array(zval *object, const char *property, uint32_t property_length, const zval *index, zval *value)
{
	...
	if (Z_TYPE_P(object) == IS_OBJECT) {
		dao_read_property(&tmp, object, property, property_length, PH_NOISY | PH_READONLY);
		...
		if (separated) {
			dao_update_property(object, property, property_length, &tmp);
			zval_ptr_dtor(&tmp);
		}
	}
...
int dao_update_property_array_str(zval *object, const char *property, uint32_t property_length, const char *index, uint32_t index_length, zval *value)
{
	...
	status = dao_update_property_array(object, property, property_length, &tmp, value);
	zval_ptr_dtor(&tmp);
	return status;
}
...
int dao_update_property_array_string(zval *object, const char *property, uint32_t property_length, zend_string *index, zval *value)
{
	zval tmp = {};
	ZVAL_STR(&tmp, index);
	return dao_update_property_array(object, property, property_length, &tmp, value);
}
...
int dao_update_property_array_merge(zval *object, const char *property, uint32_t property_length, zval *values)
{
	...
	dao_array_merge_recursive_n(&tmp, values);
 ...
 */
int dao_update_property_empty_array(zval *object, const char *property_name, uint32_t property_length) {

	zval v = {};
	int status;

	array_init(&v);
	status = dao_update_property(object, property_name, property_length, &v);
	zval_ptr_dtor(&v);
	return status;
}
...
int dao_isset_property_array(zval *object, const char *property, uint32_t property_length, const zval *index) {
	...
	if (Z_TYPE_P(object) == IS_OBJECT) {
		dao_read_property(&tmp, object, property, property_length, PH_NOISY | PH_READONLY);
		return dao_array_isset(&tmp, index);
	}
 ...
 */
int dao_read_property_array(zval *return_value, zval *object, const char *property, size_t property_length, const zval *index, int flags) {

	zval tmp = {};

	if (dao_read_property(&tmp, object, property, property_length, PH_NOISY | PH_READONLY) == FAILURE || !dao_array_isset_fetch(return_value, &tmp, index, flags)) {
		ZVAL_NULL(return_value);
		return 0;
	}
...
int dao_internal_method_exists(const zval *object, const zval *method_name){
	...
	if (ce) {
		return dao_method_exists_ce(ce, method_name);
	}
	return FAILURE;
}
...
int dao_method_exists_ce(const zend_class_entry *ce, const zval *method_name){
	...
	int res = dao_method_exists_ce_ex(ce, lcname, Z_STRLEN_P(method_name)+1);
	efree(lcname);
	return res;
}
...
int dao_create_instance_params_ce(zval *return_value, zend_class_entry *ce, zval *params)
{
	...
	if (dao_has_constructor_ce(ce)) {
		...
		outcome = dao_call_method(NULL, return_value, "__construct", param_count, params_ptr);
...
int dao_create_instance(zval *return_value, const zval *class_name){
	...
	if (unlikely(Z_TYPE_P(class_name) != IS_STRING)) {
		dao_throw_exception_string(dao_exception_ce, "Invalid class name");
		return FAILURE;
	}
	...
	return dao_create_instance_params_ce(return_value, ce, &DAO_GLOBAL(z_null));
}
...
int dao_create_instance_params(zval *return_value, const zval *class_name, zval *params){
	...
	return dao_create_instance_params_ce(return_value, ce, params);
}
...
int dao_property_incr(zval *object, const char *property_name, uint32_t property_length){
	...
	dao_read_property(&value, object, property_name, property_length, PH_READONLY);
	dao_increment(&value);
	dao_update_property(object, property_name, property_length, &value);
...
int dao_property_isset_fetch(zval *return_value, zval *object, const char *property_name, size_t property_length, int flags)
{
	zval *value;
	zend_class_entry *ce;

	if (!dao_isset_property(object, property_name, property_length)) {
		return 0;
	}
...
int dao_property_array_isset(zval *object, const char *property, size_t property_length, const zval *index)
{
	zval property_value = {};

	if (!dao_property_isset_fetch(&property_value, object, property, property_length, PH_READONLY)) {
		return 0;
	}
...
int dao_property_array_isset_fetch(zval *fetched, zval *object, const char *property, size_t property_length, const zval *index, int flags)
{
	zval property_value = {};
	...
	if (Z_TYPE(property_value) != IS_ARRAY || !dao_array_isset_fetch(fetched, &property_value, index, flags)) {
		return 0;
	}
	return 1;
}
...
int dao_property_array_pop(zval *fetched, zval *object, const char *property, size_t property_length)
{
	...
	if (!dao_array_pop(fetched, &property_value)) {
		return 0;
	}
	return 1;
}
...
int dao_property_array_last(zval *fetched, zval *object, const char *property, size_t property_length, int flags)
{
	...
	if (dao_array_last(fetched, &property_value, flags) == FAILURE) {
		return 0;
	}
	return 1;
}
