...
static zend_always_inline void __bloomfilter_calc(dao_bloomfilter *bloomfilter)
{
    uint32_t m, k;
...
static zend_always_inline void __bloomfilter_hash(dao_bloomfilter *bloomfilter, const void * key, int len)
{
    int i;
...
int dao_bloomfilter_init(dao_bloomfilter *bloomfilter, uint32_t seed, uint32_t max_items, double false_positive)
{
    ...
    dao_bloomfilter_free(bloomfilter);
...
int dao_bloomfilter_add(dao_bloomfilter *bloomfilter, const void * key, int len)
{
    ...
    for (i = 0; i < (int)bloomfilter->hash_num; i++)
    {
        DAO_BLOOMFILTER_SETBIT(bloomfilter, bloomfilter->hash_value[i]);
    }
...
int dao_bloomfilter_check(dao_bloomfilter *bloomfilter, const void * key, int len)
{
    ...
    __bloomfilter_hash(bloomfilter, key, len);
    for (i = 0; i < (int)bloomfilter->hash_num; i++) {
        if (DAO_BLOOMFILTER_GETBIT(bloomfilter, bloomfilter->hash_value[i]) == 0)
            return 1;
    }
...
int dao_bloomfilter_load(dao_bloomfilter *bloomfilter, char *filename)
{
	int ret, fd;
	FILE *fp;
	static dao_bloomfilter_file_head file_head = {0};
    ...
    if ((file_head.file_magic_code != DAO_BLOOMFILTER_FILE_MAGIC_CODE) || (file_head.bits != file_head.bytes*8)) {
		ret = -1;
        goto end;
	}
...
int dao_bloomfilter_save(dao_bloomfilter *bloomfilter, char *filename)
{
    ...
    file_head.file_magic_code = DAO_BLOOMFILTER_FILE_MAGIC_CODE;
