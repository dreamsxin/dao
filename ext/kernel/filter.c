 ...
 */
void dao_filter_alphanum(zval *return_value, zval *param){
 ...
 */
void dao_filter_identifier(zval *return_value, zval *param){
 ...
 */
void dao_is_basic_charset(zval *return_value, const zval *param){
...
static long dao_unpack(char *data, int size, int issigned, int *map)
{
	long result;
 ...
 */
static inline char *dao_longtohex(unsigned long value) {
...
void dao_escape_multi(zval *return_value, zval *param, const char *escape_char, unsigned int escape_length, char escape_extra, int use_whitelist)
{
	...
	for (i = 0; i < Z_STRLEN_P(param); i += 4) {
		...
		value |= dao_unpack(&Z_STRVAL_P(param)[i], 4, issigned, big_endian_long_map);
		if (sizeof(long) > 4) {
 ...
 */
void dao_escape_html(zval *return_value, zval *str, const zval *quote_style, const zval *charset) {
	...
	if (Z_TYPE_P(quote_style) != IS_LONG) {
		php_error_docref(NULL, E_WARNING, "Invalid quote_style supplied for dao_escape_html()");
		RETURN_ZVAL(str, 1, 0);
	}

	if (Z_TYPE_P(charset) != IS_STRING) {
		php_error_docref(NULL, E_WARNING, "Invalid charset supplied for dao_escape_html()");
		RETURN_ZVAL(str, 1, 0);
	}
