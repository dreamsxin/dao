...
void dao_strlen(zval *return_value, zval *str)
{
	...
	int flag;
	zval str_value = {};
	dao_strval(&str_value, str);
	DAO_CALL_FUNCTION_FLAG(flag, return_value, "mb_strlen", &str_value);
 ...
 *
 */
void dao_fast_join_str(zval *return_value, char *glue, unsigned int glue_length, zval *pieces){
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(pieces), tmp) {
		dao_append_printable_zval(&implstr, tmp);
		if (++i != numelems) {
...
void dao_pad_str(zval *return_value, zval *input, int pad_length, const char *pad_str, int pad_type)
{
	...
	if (pad_type < DAO_PDA_LEFT || pad_type > DAO_PDA_BOTH) {
...
void dao_random_string(zval *return_value, const zval *type, const zval *length) {
	...
	if (Z_LVAL_P(type) > DAO_RANDOM_NOZERO) {
		return;
	}
...
void dao_substr(zval *return_value, zval *str, unsigned long from, long length) {
	...
	if (Z_TYPE_P(str) != IS_STRING) {
		...
		if (Z_TYPE_P(str) == IS_NULL || DAO_IS_BOOL(str)) {
			ZVAL_FALSE(return_value);
			return;
		}
		...
		zend_error(E_WARNING, "Invalid arguments supplied for dao_substr()");
		ZVAL_FALSE(return_value);
		return;
	}
...
void dao_append_printable_array(smart_str *implstr, zval *value) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(value), tmp) {
		...
		if (Z_TYPE_P(tmp) == IS_OBJECT) {
			...
			if (Z_TYPE_P(tmp) == IS_ARRAY) {
				dao_append_printable_array(implstr, tmp);
			} else {
				dao_append_printable_zval(implstr, tmp);
			}
		}
...
void dao_unique_key(zval *return_value, zval *prefix, zval *value) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		dao_append_printable_array(&implstr, value);
	} else {
		dao_append_printable_zval(&implstr, value);
	}
...
int dao_preg_match(zval *retval, zval *regex, zval *subject, zval *matches, zend_long flags, int global)
{
...
int dao_json_encode(zval *retval, zval *v, int opts)
{
	int flag;
...
# endif
	...
	DAO_CALL_FUNCTION_FLAG(flag, retval, "json_encode", v, &zopts);
#endif
	return flag;
}

int dao_json_decode(zval *retval, zval *v, zend_bool assoc)
{
...
# endif
#else
	zval *zassoc = assoc ? &DAO_GLOBAL(z_true) : &DAO_GLOBAL(z_false);
	zval *params[] = { v, zassoc };

	return dao_call_function_with_params(retval, SL("json_decode"), 2, params);
#endif
}
...
void dao_date(zval *return_value, zval *format, zval *timestamp)
{
	...
	ts = (timestamp) ? dao_get_intval(timestamp) : time(NULL);
...
void dao_add_trailing_slash(zval* return_value, zval *v)
{
	...
	DAO_ENSURE_IS_STRING(return_value);
