...
void *dao_message_queue_message_alloc_blocking(struct dao_message_queue *queue) {
	void *rv = dao_message_queue_message_alloc(queue);
	while(!rv) {
		__sync_fetch_and_add(&queue->allocator.blocked_readers, 1);
		rv = dao_message_queue_message_alloc(queue);
		if(rv) {
			__sync_fetch_and_add(&queue->allocator.blocked_readers, -1);
			return rv;
		}
...
void *dao_message_queue_read(struct dao_message_queue *queue) {
	void *rv = dao_message_queue_tryread(queue);
	while(!rv) {
		__sync_fetch_and_add(&queue->queue.blocked_readers, 1);
		rv = dao_message_queue_tryread(queue);
		if(rv) {
			__sync_fetch_and_add(&queue->queue.blocked_readers, -1);
			return rv;
		}
