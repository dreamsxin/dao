...
static int dao_thread_pool_empty(dao_thread_pool_t *pool)
{
	int i;
	for (i = 0; i < pool->num_threads; i++) {
		if (!dao_thread_pool_queue_empty(&pool->threads[i]))
			return 0;
	}
	return 1;
}

static dao_thread_pool_thread_t* round_robin_schedule(dao_thread_pool_t *pool)
{
	static int cur_thread_index = -1;
	dao_thread_pool_thread_t *thread;
...
static dao_thread_pool_thread_t* least_load_schedule(dao_thread_pool_t *pool)
{
	int i;
	...
	for (i = 1; i < pool->num_threads; i++) {
		if (dao_thread_pool_queue_len(&pool->threads[i]) < dao_thread_pool_queue_len(&pool->threads[min_num_works_index])) {
...
static const dao_thread_pool_schedule_func schedule_alogrithms[] = {
	[DAO_THREAD_POOL_ROUND_ROBIN] = round_robin_schedule,
	[DAO_THREAD_POOL_LEAST_LOAD]  = least_load_schedule
};
...
static dao_thread_pool_work_t *get_work_concurrently(dao_thread_pool_thread_t *thread)
{
	dao_thread_pool_work_t *work = NULL;
	unsigned int tmp;

	do {
		work = NULL;
		if (dao_thread_pool_queue_len(thread) <= 0)
			break;
		tmp = thread->out;
		//prefetch work
		work = &thread->work_queue[dao_thread_pool_queue_offset(tmp)];

		//printf("\nget_work_concurrently:%p,i:%d\n", (void*)thread->id, dao_thread_pool_queue_offset(tmp));
...
static void *dao_thread_pool_thread_start_routine(void *arg)
{
	dao_thread_pool_thread_t *thread = arg;
	dao_thread_pool_t *pool = thread->pool;
	dao_thread_pool_work_t *work = NULL;

	__sync_fetch_and_add(&pool->num_threads, 1);

	while (1) {		
		while (dao_thread_pool_queue_empty(thread) && !thread->shutdown) {
		...
		if (unlikely(DAO_GLOBAL(debug).enable_debug)) {
			DAO_THREAD_POOL_DEBUG("I'm awake");
		}

		if (thread->shutdown) {
			pthread_cond_signal(&pool->cond);
			if (unlikely(DAO_GLOBAL(debug).enable_debug)) {
				DAO_THREAD_POOL_DEBUG("exit! %ld: %d\n", (*(unsigned long*)&(thread->id)), thread->num_works_done);
			}
			pthread_exit(NULL);
		}
		...
		if (work) {
			//DAO_THREAD_POOL_DEBUG("call task");
			//zend_print_zval_r(&work->args, 0);
			dao_call_user_func_array(NULL, &work->routine, &work->args);
...
static int spawn_new_thread(dao_thread_pool_t *pool, int index)
{
	memset(&pool->threads[index], 0, sizeof(dao_thread_pool_thread_t));
	...
	if (pthread_create(&pool->threads[index].id, NULL, dao_thread_pool_thread_start_routine, (void *)(&pool->threads[index])) != 0) {
...
dao_thread_pool_t *dao_thread_pool_init(int num_threads)
{
	int i;
	...
	if (num_threads <= 0) {
		return NULL;
	} else if (num_threads > DAO_THREAD_POOL_MAX_NUM) {
		zend_error(E_ERROR, "Too many threads!");
		return NULL;
	}
	pool = malloc(sizeof(dao_thread_pool_t));
	if (pool == NULL) {
		zend_error(E_ERROR, "Malloc failed!");
		return NULL;
	}

	memset(pool, 0, sizeof(dao_thread_pool_t));
	pool->num_threads = 0;
	...
	for (i = 0; i < num_threads; i++) {
		if (spawn_new_thread(pool, i) < 0) {
			//exit(-1);
			dao_thread_pool_destroy(pool, 0);
			return NULL;
		}
	}
...
static int dispatch_work2thread(dao_thread_pool_t *pool, dao_thread_pool_thread_t *thread, zval *routine, zval *args)
{
	dao_thread_pool_work_t *work = NULL;

	if (dao_thread_pool_queue_full(thread)) {
		zend_error(E_NOTICE, "Queue of thread selected is full!");
		return -1;
	}
	work = &thread->work_queue[dao_thread_pool_queue_offset(thread->in)];
	zval_ptr_dtor(&work->routine);
	zval_ptr_dtor(&work->args);
	...
	for (i =0; i <= thread->in; i++) {
		printf("\ni:%d\n", dao_thread_pool_queue_offset(i));
		work = &thread->work_queue[dao_thread_pool_queue_offset(i)];
		zend_print_zval_r(&work->args, 0);
	}
	*/
	thread->in++;
...
*/
static int migrate_thread_work(dao_thread_pool_t *pool, dao_thread_pool_thread_t *from)
{
	unsigned int i;
	dao_thread_pool_work_t *work;
	dao_thread_pool_thread_t *to;

	for (i = from->out; i < from->in; i++) {
		work = &from->work_queue[dao_thread_pool_queue_offset(i)];
...
#ifdef DAO_DEBUG
	printf("%ld migrate_thread_work: %u\n", from->id, dao_thread_pool_queue_len(from));
#endif
	return 0;
}
 ...
 * real-time
*/
static void balance_thread_load(dao_thread_pool_t *pool)
{
	int count[DAO_THREAD_POOL_MAX_NUM];
	int i, sum = 0, avg;
	int first_neg_id, first_pos_id, tmp, migrate_num;
	dao_thread_pool_thread_t *from, *to;
	dao_thread_pool_work_t *work;

	for (i = 0; i < pool->num_threads; i++) {
		count[i] = dao_thread_pool_queue_len(&pool->threads[i]);
		sum += count[i];
	}
...
int dao_thread_pool_inc_threads(dao_thread_pool_t *pool, int num_inc)
{
	...
	if (num_threads > DAO_THREAD_POOL_MAX_NUM) {
...
int dao_thread_pool_dec_threads(dao_thread_pool_t *pool, int num_dec)
{
	...
	if (pool->num_threads == 0 && !dao_thread_pool_empty(pool)) {
...
void dao_thread_pool_destroy(dao_thread_pool_t *pool, int finish)
{
	...
	if (finish == 1) {
		if (unlikely(DAO_GLOBAL(debug).enable_debug)) {
			DAO_THREAD_POOL_DEBUG("Wait all work done");
		}

		while (!dao_thread_pool_empty(pool)) {
			struct timespec abstime;
			format_wait_time(1000, &abstime);
	...
	if (unlikely(DAO_GLOBAL(debug).enable_debug)) {
		DAO_THREAD_POOL_DEBUG("wait worker thread exit");
	}
	for (i = 0; i < pool->num_threads; i++) {
		dao_thread_pool_thread_t *thread = &pool->threads[i];
		if (pool->threads[i].id) {
			pthread_join(pool->threads[i].id, NULL);
		}
		...
		while (!dao_thread_pool_queue_empty(thread)) {
			dao_thread_pool_work_t *work = get_work_concurrently(thread);
			if (work) {
				zval_ptr_dtor(&work->routine);
				zval_ptr_dtor(&work->args);
			}
