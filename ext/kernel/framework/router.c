...
zval *dao_replace_marker(int named, zval *paths, zval *replacements, unsigned long *position, char *cursor, char *marker)
{
 ...
 */
void dao_replace_paths(zval *return_value, zval *pattern, zval *paths, zval *replacements){
	...
	if (Z_TYPE_P(pattern) != IS_STRING || Z_TYPE_P(replacements) != IS_ARRAY || Z_TYPE_P(paths) != IS_ARRAY) {
		...
		php_error_docref(NULL, E_WARNING, "Invalid arguments supplied for dao_replace_paths()");
		return;
	}
		...
		if (parentheses_count == 0 && !looking_placeholder) {
			if (ch == '{') {
				...
				if (ch == '}') {
					...
					if (intermediate > 0) {
						if (bracket_count == 0) {
							replace = dao_replace_marker(1, paths, replacements, &position, cursor, marker);
							if (replace) {
								use_copy = 0;
		...
		if (bracket_count == 0 && !looking_placeholder) {
			if (ch == '(') {
				...
				if (ch == ')') {
					...
					if (intermediate > 0) {
						if (parentheses_count == 0) {
							replace = dao_replace_marker(0, paths, replacements, &position, cursor, marker);
							if (replace) {
								use_copy = 0;
								if (Z_TYPE_P(replace) != IS_STRING) {
									use_copy = zend_make_printable_zval(replace, &replace_copy);
 ...
 */
void dao_extract_named_params(zval *return_value, zval *str, zval *matches)
{
	zval tmp = {};
	int i, k;
