 ...
 *------------------------------------
 *
 * Dao uses memory frames to track the variables used within a method
 ...
 * memory frame is globally accesed using DAO_GLOBAL(start_frame)
 *
 * Not all methods must grow/restore the dao_memory_entry.
 */

void dao_initialize_memory(zend_dao_globals *dao_globals_ptr)
{
	ZVAL_NULL(&dao_globals_ptr->z_null);
	ZVAL_FALSE(&dao_globals_ptr->z_false);
	ZVAL_TRUE(&dao_globals_ptr->z_true);
	ZVAL_LONG(&dao_globals_ptr->z_zero, 0);
	ZVAL_LONG(&dao_globals_ptr->z_one, 1);
	ZVAL_LONG(&dao_globals_ptr->z_two, 2);

	dao_globals_ptr->initialized = 1;
}
...
void dao_deinitialize_memory()
{
	...
	if (dao_globals_ptr->initialized != 1) {
		dao_globals_ptr->initialized = 0;
		return;
	}
