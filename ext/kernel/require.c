...
static int dao_valid_var_name(char *var_name, int len) /* {{{ */
{
	int i, ch;
...
zend_array *dao_build_symtable(zval *vars) {
	...
	if (vars && Z_TYPE_P(vars) == IS_ARRAY) {
	    ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(vars), var_name, entry) {
			...
			if (dao_valid_var_name(ZSTR_VAL(var_name), ZSTR_LEN(var_name))) {
...
int dao_exec_file(zval *ret, zval *object, zval *file, zval *vars, zend_array *symbol_table) {
	...
	if (symbol_table == NULL) {
		...
		ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(vars), var_name, entry) {
			...
			if (var_name) {	
				...
				dao_set_symbol(symbol_table, &key, entry);
			}
		} ZEND_HASH_FOREACH_END();
	}
	...
	if (op_array) {
		...
		status = _dao_exec(ret, object, op_array, symbol_table);
	...
	if (destroy) {
		dao_destroy_symtable(symbol_table);
	}
	return status;
}
...
int dao_exec_code(zval *ret, zval *object, zval *code, zval * vars) {
	...
	if (Z_STRLEN_P(code)) {
		...
		if (op_array) {
			(void)_dao_exec(ret, object, op_array, symbol_table);
			destroy_op_array(op_array);
			efree(op_array);
		}
	}
 ...
 */
int dao_require_ret(zval *return_value_ptr, const char *require_path)
{
	zend_file_handle file_handle;
