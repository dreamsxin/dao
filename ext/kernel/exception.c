 ...
 */
void dao_throw_exception(zval *object){
...
void dao_throw_exception_debug(zval *object, const char *file, uint32_t line)
{
	...
	if (!object || Z_TYPE_P(object) != IS_OBJECT) {
		...
		DAO_CALL_METHOD(NULL, &exception, "__construct", &DAO_GLOBAL(z_null), &DAO_GLOBAL(z_zero), object);
	} else {
		ZVAL_COPY_VALUE(&exception, object);
	}

	if (line > 0) {
		DAO_CALL_METHOD(&curline, &exception, "getline");
		if (DAO_IS_LONG(&curline, 0)) {
 ...
 */
void dao_throw_exception_string(zend_class_entry *ce, const char *message){
...
void dao_throw_exception_string_debug(zend_class_entry *ce, const char *message, uint32_t message_len, const char *file, uint32_t line)
{
	...
	DAO_CALL_METHOD(NULL, &object, "__construct", &DAO_GLOBAL(z_null), &msg);
 ...
 */
void dao_throw_exception_zval(zend_class_entry *ce, zval *message){

	zval object = {};
	object_init_ex(&object, ce);

	DAO_CALL_METHOD(NULL, &object, "__construct", message);
	zend_throw_exception_object(&object);
}
...
void dao_throw_exception_zval_debug(zend_class_entry *ce, zval *message, const char *file, uint32_t line){
	...
	DAO_CALL_METHOD(NULL, &object, "__construct", &DAO_GLOBAL(z_null), message);
...
void dao_throw_exception_format(zend_class_entry *ce, const char *format, ...) {
	...
	DAO_CALL_METHOD(NULL, &object, "__construct", &msg);
