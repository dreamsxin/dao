...
void php_dao_init_globals(zend_dao_globals *dao_globals) {
	...
	dao_globals->initialized = 0;

	/* Recursive Lock */
	dao_globals->recursive_lock = 0;

	/* ORM options*/
	dao_globals->orm.events = 1;
	dao_globals->orm.virtual_foreign_keys = 1;
	dao_globals->orm.not_null_validations = 1;
	dao_globals->orm.length_validations = 1;
	dao_globals->orm.use_mb_strlen = 1;
	dao_globals->orm.exception_on_failed_save = 0;
	dao_globals->orm.enable_literals = 1;
	dao_globals->orm.cache_level = 3;
	dao_globals->orm.ast_cache = NULL;
	dao_globals->orm.enable_property_method = 1;
	dao_globals->orm.enable_auto_convert = 1;
	dao_globals->orm.allow_update_primary = 0;
	dao_globals->orm.enable_strict = 0;
	dao_globals->orm.must_column = 1;

	/* Security options */
	dao_globals->security.crypt_std_des_supported  = zend_hash_str_exists(constants, SL("CRYPT_STD_DES"));
	dao_globals->security.crypt_ext_des_supported  = zend_hash_str_exists(constants, SL("CRYPT_EXT_DES"));
	dao_globals->security.crypt_md5_supported      = zend_hash_str_exists(constants, SL("CRYPT_MD5"));
	dao_globals->security.crypt_blowfish_supported = zend_hash_str_exists(constants, SL("CRYPT_BLOWFISH"));
	dao_globals->security.crypt_sha256_supported   = zend_hash_str_exists(constants, SL("CRYPT_SHA256"));
	dao_globals->security.crypt_sha512_supported   = zend_hash_str_exists(constants, SL("CRYPT_SHA512"));

	dao_globals->security.crypt_blowfish_y_supported = dao_globals->security.crypt_blowfish_supported;

	/* DB options */
	dao_globals->db.escape_identifiers = 1;
}
 ...
 */
int dao_function_exists_ex(const char *method_name, unsigned int method_len) {
...
int dao_fetch_parameters(int num_args, int required_args, int optional_args, ...)
{
	...
	if (num_args < required_args) {
		dao_throw_exception_format(spl_ce_BadMethodCallException, "Wrong number of parameters, num:%d, required:%d", num_args, required_args);
		return FAILURE;
	}

	if (num_args > arg_count) {
		dao_throw_exception_format(spl_ce_BadMethodCallException, "Could not obtain parameters for parsing, num:%d, count:%d", num_args, arg_count);
		return FAILURE;
	}
