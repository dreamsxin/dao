 ...
 */
void dao_fast_filemtime(zval *return_value, zval *filename){
...
void dao_fix_path(zval *return_value, zval *path, zval *directory_separator) {
	...
	if (Z_STRLEN_P(path) > 0 && Z_STRLEN_P(directory_separator) > 0) {
		if (Z_STRVAL_P(path)[Z_STRLEN_P(path) - 1] != Z_STRVAL_P(directory_separator)[0]) {
			DAO_CONCAT_VV(return_value, path, directory_separator);
			return;
		}
	}
...
/**
 * Faster version of dao_prepare_virtual_path()
 */
...
void dao_file_get_contents(zval *return_value, zval *filename)
{
	...
	if (Z_TYPE_P(filename) != IS_STRING) {
		php_error_docref(NULL, E_WARNING, "Invalid arguments supplied for dao_file_get_contents()");
		RETVAL_FALSE;
		return;
	}
...
void dao_file_put_contents(zval *return_value, zval *filename, zval *data)
{
	...
	if (Z_TYPE_P(filename) != IS_STRING) {
		php_error_docref(NULL, E_WARNING, "Invalid arguments supplied for dao_file_put_contents()");
		if (return_value) {
			RETVAL_FALSE;
		}
		return;
...
void dao_fwrite(zval *return_value, zval *stream_zval, zval *data)
{
	...
	if (Z_TYPE_P(stream_zval) != IS_RESOURCE) {
		php_error_docref(NULL, E_WARNING, "Invalid arguments supplied for dao_fwrite()");
		if (return_value) {
			RETVAL_FALSE;
		} else {
			return;
...
int dao_feof(zval *stream_zval)
{
	...
	if (Z_TYPE_P(stream_zval) != IS_RESOURCE) {
		php_error_docref(NULL, E_WARNING, "Invalid arguments supplied for dao_feof()");
		return 0;
	}
...
{
#ifdef PHP_WIN32
	return Z_STRLEN_P(path) >= 3 && Z_STRVAL_P(path)[1] == ':' && Z_STRVAL_P(path)[2] == DAO_DIRECTORY_SEPARATOR[0];
#else
