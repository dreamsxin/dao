...
dao_shared_memory* dao_shared_memory_create(char const* name, size_t sz)
{
	int rc = 0;
	dao_shared_memory* shma = emalloc(sizeof(*shma));
...
error:
	dao_shared_memory_cleanup(shma);
	errno = rc;
	return NULL;
}
...
void dao_shared_memory_cleanup(dao_shared_memory* src)
{
	...
	dao_shared_memory* shma = (dao_shared_memory*)src;
