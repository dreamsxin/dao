...
static void phql_ret_insert_statement2(zval *ret, zval *Q, zval *V)
{
	...
	if (!dao_array_isset_fetch_str(&values, ret, ISL(values), PH_READONLY)) {
		array_init(&values);
		add_assoc_zval(ret, ISV(values), &values);
	}
	add_next_index_zval(&values, V);
...
int phql_parse_phql(zval *result, zval *phql) {
	...
	if (Z_TYPE_P(phql) != IS_STRING) {
		DAO_THROW_EXCEPTION_STR(dao_mvc_model_exception_ce, "PHQL is must be string");
		return FAILURE;
	}

	if (phql_internal_parse_phql(result, Z_STRVAL_P(phql), Z_STRLEN_P(phql), &error_msg) == FAILURE) {
		if (Z_TYPE(error_msg) > IS_NULL) {
			DAO_THROW_EXCEPTION_STR(dao_mvc_model_exception_ce, Z_STRVAL(error_msg));
			zval_ptr_dtor(&error_msg);
		}
		else {
			DAO_THROW_EXCEPTION_STR(dao_mvc_model_exception_ce, "There was an error parsing PHQL");
		}
...
int phql_internal_parse_phql(zval *result, char *phql, unsigned int phql_length, zval *error_msg) {
	...
	dao_orm_get_prepared_ast(result, &unique_id);
	...
	parser_status->enable_literals = dao_globals_ptr->orm.enable_literals;
	...
	if (status != FAILURE) {
		if (parser_status->status == PHQL_PARSING_OK) {
			if (Z_TYPE(parser_status->ret) == IS_ARRAY) {
				 ...
				 */
				if (dao_globals_ptr->orm.cache_level >= 1) {
				 ...
				 */
				dao_orm_set_prepared_ast(&unique_id, result);
