...
{
	dao_py_object_object *pip;

	PHP_PYTHON_THREAD_ASSERT();

	/* Create a new instance of a PHP Python object. */
	if (object_init_ex(zv, dao_py_object_ce) != SUCCESS)
		return FAILURE;
	 ...
	 */
	pip = dao_py_object_object_from_obj(Z_OBJ_P(zv));
	Py_INCREF(o);
	pip->obj = o;
...
static PyObject *php_call(PyObject *self, PyObject *args)
{
	...
	if (dao_call_user_func_args(&ret, &lcname, argv, argc) != SUCCESS) {
...
int python_php_init()
{
	if (Py_InitModule3("phalcon", python_php_methods, "Dao7 Module") == NULL)
		return FAILURE;
