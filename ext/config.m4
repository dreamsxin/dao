PHP_ARG_ENABLE(dao, whether to enable Dao framework,
[  --enable-dao        Enable Dao framework])

PHP_ARG_ENABLE(dao-debug, for Dao debug support,
[  --enable-dao-debug  Enable enable Dao debug support], no, no)

if test "$PHP_DAO_DEBUG" != "no"; then
	CFLAGS="$CFLAGS -Wall -g3 -ggdb -O0 -DDAO_DEBUG=1"
	AC_DEFINE(DAO_DEBUG, 1, [Enable Dao debug support])
else
	CFLAGS="$CFLAGS -DDAO_RELEASE=1"
fi
...
[  --enable-storage-btree   Enable storage btree support], yes, no)
...
if test "$PHP_DAO" = "yes"; then
	...
	AC_DEFINE(HAVE_DAO, 1, [Whether you have Dao Framework])
	dao_sources="dao.c \
kernel/main.c \
kernel/fcall.c \
kernel/require.c \
...
aop.c"
	...
	if test "$PHP_STORAGE_BTREE" = "yes"; then
		AC_DEFINE([DAO_STORAGE_BTREE], [1], [Whether storage btree are available])
		dao_sources="$dao_sources storage/btree/bplus.c storage/btree/pages.c storage/btree/utils.c storage/btree/values.c storage/btree/writer.c storage/btree.c"
	fi
	...
	AC_CHECK_HEADERS(
		...
		[
			PHP_ADD_EXTENSION_DEP([dao], [igbinary])
			AC_DEFINE([DAO_USE_PHP_IGBINARY], [1], [Whether PHP igbinary extension is present at compile time])
		],
		,
		[[#include "main/php.h"]]
	)
	...
	AC_CHECK_HEADERS(
		[ext/pcre/php_pcre.h],
		[
			PHP_ADD_EXTENSION_DEP([dao], [pcre])
			AC_DEFINE([DAO_USE_PHP_PCRE], [1], [Whether PHP pcre extension is present at compile time])
		],
		[
	...
	AC_CHECK_HEADERS(
		[ext/json/php_json.h],
		[
			PHP_ADD_EXTENSION_DEP([dao], [json])
			AC_DEFINE([DAO_USE_PHP_JSON], [1], [Whether PHP json extension is present at compile time])
		],
		,
		[[#include "main/php.h"]]
	)
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				...
				[
					PHP_ADD_EXTENSION_DEP([dao], [session])
					AC_DEFINE([DAO_USE_PHP_SESSION], [1], [Whether PHP session extension is present at compile time])
				],
				,
				[[#include "main/php.h"]]
			)
	...
	AC_CHECK_HEADERS(
		...
		[
			PHP_SOCKETS="yes"
			PHP_ADD_EXTENSION_DEP([dao], [sockets])
			AC_DEFINE([DAO_USE_PHP_SOCKETS], [1], [Whether PHP sockets extension is present at compile time])
		],
		,
		[[#include "main/php.h"]]
	)
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				[ext/hash/php_hash.h],
				[
					PHP_ADD_EXTENSION_DEP([dao], [hash])
					AC_DEFINE([DAO_USE_PHP_HASH], [1], [Whether PHP hash extension is present at compile time])
				],
				,
				[[#include "main/php.h"]]
			)
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				...
				[
					PHP_ADD_EXTENSION_DEP([dao], [mbstring])
					AC_DEFINE([DAO_USE_PHP_MBSTRING], [1], [Whether PHP mbstring extension is present at compile time])
				],
				,
				[[#include "main/php.h"]]
			)
	...
	if pkg-config --exists libpng; then
		...
		PHP_EVAL_LIBLINE($PNG_LIBS, DAO_SHARED_LIBADD)


		AC_MSG_RESULT(yes)

		AC_DEFINE([DAO_USE_PNG], [1], [Have libpng support])
	fi
	...
	for i in /usr/local /usr; do
		if test -r $i/include/ImageMagick/wand/MagickWand.h; then
			...
			AC_DEFINE([DAO_USE_MAGICKWAND], [1], [Have ImageMagick MagickWand support])
			break
	...
	for i in /usr/local /usr; do
		if test -r $i/include/libmongoc-1.0/mongoc.h; then
			...
			AC_DEFINE([DAO_USE_MONGOC], [1], [Have libmongoc support])
			dao_sources="$dao_sources cache/backend/mongo.c mvc/model/metadata/mongo.c"

			PHP_CHECK_LIBRARY(mongoc-1.0, mongoc_collection_find_with_opts,
			[
				AC_DEFINE(DAO_MONGOC_HAS_FIND_OPTS, 1, [Has mongoc_collection_find_with_opts support])
			])
			break
		else
			AC_MSG_RESULT([no, found in $i])
	...
	if test "$PHP_ASYNC" = "yes" && test "$dao_php_version" -ge "7001000"; then
		AC_MSG_RESULT(yes)
	...
	PHP_SETUP_OPENSSL(DAO_SHARED_LIBADD, [
	...
	if test "$PHP_JWT" = "yes" && test "$PHP_OPENSSL" = "yes"; then
		AC_DEFINE([DAO_JWT], [1], [Whether JWT are available])
	...
	if test "$async_use_ucontext" = "yes"; then
		...
		dao_sources="$dao_sources $async_source_files $UV_SRC"
		AC_DEFINE(DAO_USE_ASYNC, 1, [Have async support])
	fi

	AC_MSG_CHECKING([Include non-free minifiers])
	if test "$PHP_NON_FREE" = "yes"; then
		AC_MSG_RESULT([yes])
		AC_DEFINE([DAO_NON_FREE], [1], [Whether non-free minifiers are available])
		dao_sources="$dao_sources assets/filters/jsminifier.c assets/filters/cssminifier.c "
	else
		AC_MSG_RESULT([no])
		dao_sources="$dao_sources assets/filters/nojsminifier.c assets/filters/nocssminifier.c "
	fi
	...
	if test "$PHP_STORAGE_WIREDTIGER" = "yes"; then
		...
		for i in /usr/local /usr; do
			if test -r $i/include/wiredtiger.h; then
				...
				[
					PHP_ADD_LIBRARY_WITH_PATH(wiredtiger, $i/$PHP_LIBDIR, DAO_SHARED_LIBADD)
					AC_DEFINE(DAO_USE_WIREDTIGER, 1, [Have wiredtiger support])
					dao_sources="$dao_sources storage/wiredtiger.c storage/wiredtiger/pack.c storage/wiredtiger/cursor.c cache/backend/wiredtiger.c "
				],[
	...
	if test "$PHP_STORAGE_LMDB" = "yes"; then
		AC_DEFINE(DAO_USE_LMDB, 1, [Have lmdb support])
		dao_sources="$dao_sources storage/lmdb.c storage/lmdb/cursor.c storage/lmdb/mdb.c storage/lmdb/midl.c cache/backend/lmdb.c "
	fi
	...
	if test "$PHP_STORAGE_LIBMDBX" = "yes"; then
		...
		for i in /usr/local /usr; do
			if test -r $i/include/mdbx.h; then
				...
				[
					PHP_ADD_LIBRARY_WITH_PATH(mdbx, $i/$PHP_LIBDIR, DAO_SHARED_LIBADD)
					AC_DEFINE(DAO_USE_LIBMDBX, 1, [Have libmdbx support])
					dao_sources="$dao_sources storage/libmdbx.c storage/libmdbx/cursor.c "
				],[
	...
	if test "$PHP_STORAGE_LEVELDB" = "yes"; then
		...
		for i in /usr/local /usr; do
			if test -r $i/include/leveldb/c.h; then
				...
				[
					PHP_ADD_LIBRARY_WITH_PATH(leveldb, $i/$PHP_LIBDIR, DAO_SHARED_LIBADD)
					AC_DEFINE(DAO_USE_LEVELDB, 1, [Have leveldb support])
					dao_sources="$dao_sources storage/leveldb.c storage/leveldb/writebatch.c storage/leveldb/iterator.c "
				],[
	...
	if test "$PHP_STORAGE_BLOOMFILTER" = "yes"; then
		AC_DEFINE(DAO_USE_BLOOMFILTER, 1, [Have bloomfilter support])
		dao_sources="$dao_sources storage/bloomfilter.c storage/bloomfilter/counting.c "
	fi

	if test "$PHP_STORAGE_DATRIE" = "yes"; then
		AC_DEFINE(DAO_USE_DATRIE, 1, [Have datrie support])
		dao_sources="$dao_sources storage/datrie.c "
	fi
	...
	AC_TRY_LINK([
		...
		AC_DEFINE([DAO_USE_SHM_OPEN], 1, [Have shm_open support])
		AC_MSG_RESULT([yes])

		dao_sources="$dao_sources sync/exception.c sync/mutex.c sync/readerwriter.c sync/event.c sync/semaphore.c sync/sharedmemory.c"
	], [
	...
	PHP_ADD_LIBRARY(dl)
	PHP_CHECK_LIBRARY(rt, clock_gettime, [PHP_ADD_LIBRARY(rt,, DAO_SHARED_LIBADD)])
	...
	if test "$PHP_PYTHON" == "yes"; then
		...
		AC_TRY_LINK([
			...
			AC_DEFINE([DAO_USE_PYTHON], 1, [Have python support])
			AC_MSG_RESULT([yes])

			dao_sources="$dao_sources py/common.c py.c py/object.c py/exception.c py/matplot.c"
		],[
			AC_MSG_ERROR([no])
		])
		...
		AC_CHECK_HEADERS(
			[numpy/arrayobject.h],
			[
				AC_DEFINE([DAO_USE_PYTHON_NUMPY], [1], [Have python numpy support])
			],
			,
			[[#include <Python.h>]]
		)
	...
	PHP_NEW_EXTENSION(dao, $dao_sources, $ext_shared,, \\$(ASYNC_CFLAGS))
	PHP_SUBST(ASYNC_CFLAGS)
	PHP_SUBST(DAO_SHARED_LIBADD)
	PHP_ADD_EXTENSION_DEP([dao], [spl])
	PHP_ADD_EXTENSION_DEP([dao], [date])
