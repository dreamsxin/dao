...
$token = \Dao\JWT::encode($payload, $hmackey);
print_r($token.PHP_EOL);
$decoded_token = \Dao\JWT::decode($token, $hmackey);
print_r($decoded_token);

$payload = ['data' => 'data', 'exp' => time() - 1];

$token = \Dao\JWT::encode($payload, $hmackey, 'HS256');

try {
	$decoded_token = \Dao\JWT::decode($token, $hmackey, ['algorithm' => 'HS256']);
	print_r($decoded_token);
} catch (\Exception $e) {
	print_r($e);
}
...
try {
	$decoded_token = \Dao\JWT::decode($token, $hmackey, ['aud' => ['Young'], 'algorithm' => 'HS256']);
} catch (\Exception $e) {
	print_r($e);
}
...
try {
	$decoded_token = \Dao\JWT::decode($token, $hmackey, ['jti' => md5('id'), 'algorithm' => 'HS256']);
} catch (\Exception $e) {
	print_r($e);
}
...
try {
	$decoded_token = \Dao\JWT::decode($token, $hmackey, ['leeway' => 30, 'algorithm' => 'HS256']);
	print_r($decoded_token);
} catch (\Exception $e) {
	print_r($e);
}
