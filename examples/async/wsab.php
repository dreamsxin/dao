...
$requests = Dao\Arr::get($vals, 'requests');
if ($requests <= 0) {
	$requests = 1;
}
$concurrency = Dao\Arr::get($vals, 'concurrency');
if ($concurrency <= 0) {
	$concurrency = 1;
}
...
$context = \Dao\Async\Context::background();
...
$channel = new \Dao\Async\Channel(0);

for ($j=0; $j<$concurrency; $j++)  {
	$tasks[] = \Dao\Async\Task::asyncWithContext($context, static function (iterable $it) use ($context, $work, &$total_request, &$connect_success, &$connect_fail, &$send_success, &$send_fail, &$recv_success, &$recv_fail, &$min_request_time, &$max_request_time) {
		foreach ($it as $url) {
