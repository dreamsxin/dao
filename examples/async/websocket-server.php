...
class Websocket
{
	...
	public function start()
	{
		...
		$worker = static function ($socket) use ($ws, $callback) {
			...
			$socket->isHttp = false;
			$socket->parser = new \Dao\Http\Parser();
			...
			try {
				...
				while (!$socket->is_closing && null !== ($chunk = $socket->read())) {
					...
					if ($socket->isHttp) {
						...
						if ($socket->parser->status() == \Dao\Http\Parser::STATUS_END) {
		...
		try {
			...
			while (true) {
				...
				// \Dao\Async\Task::async($worker, $socket);
...
$ws = new Websocket(\Dao\Arr::get($vals, 'server', '0.0.0.0'), \Dao\Arr::get($vals, 'port', 10001), function($socket, $headers, $path, $data) {
	...
	if ($path) {
		...
		$handlerName = Dao\Text::camelize($handlerName);
		$handlerName .= 'Plugin';
		$actionName .= 'Action';
