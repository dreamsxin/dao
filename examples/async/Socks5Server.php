				...
				while (!$socket->is_closing && null !== ($chunk = $socket->read())) {
					...
					switch ($socket->status) {
						...
						case Socks5::STATUS_ADDR:
							...
							$socket->client = \Dao\Async\Network\TcpSocket::connect($headers[1], $headers[2], $tls);
							$socket->write(Socks5::REPLY_ADDR);
							$socket->status  = Socks5::STATUS_STREAM;
	
							\Dao\Async\Task::async(function ($client) use ($socket) {
...
if (isset($vals['debug'])) {
	...
	echo Dao\Cli\Color::info('Use debug mode').PHP_EOL;
}
$sserver = new Socks5Server(\Dao\Arr::get($vals, 'server', '0.0.0.0'), \Dao\Arr::get($vals, 'port', 10002), function($socket, $status, $data) {
	// TODO
