...
function sslpair(): array
{
    ...
    $tls = new Dao\Async\Network\TlsServerEncryption();
    $tls = $tls->withDefaultCertificate($file . 'crt', $file . 'key', '123456');

    $server = Dao\Async\Network\TcpServer::listen('localhost', 0, $tls);

    $tasks = [
         Dao\Async\Task::async(function () use ($server) {
            try {
         ...
         Dao\Async\Task::async(function () use ($server) {
            ...
            $socket = Dao\Async\Network\TcpSocket::connect($server->getAddress(), $server->getPort(), $tls);
    ...
    return Dao\Async\Task::await(Dao\Async\Deferred::combine($tasks, function (Dao\Async\Deferred $defer, bool $last, $k, ?\Throwable $e, $v = null) use (& $result) {
        if ($e) {
            $defer->fail($e);
        } else {
            $result[$k] = $v;
...
Dao\Async\Task::async(function () use ($a, $async, $chunkSize, $count) {
    try {
